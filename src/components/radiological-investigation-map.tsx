      ...
      <div className="radiological-map-grid">
        ...
        <aside className="radiological-map-list" aria-label="Radiological map records">
          ...
          <div className="radiological-options">
            {displayedHistorical.length > 0 && <details className="radiological-survey-table"><summary>{displayedHistorical.length} historical survey points — view as table</summary><div role="table" aria-label="Filtered historical survey points">{displayedHistorical.map((record) => <button role="row" type="button" key={record.id} className={selection.type === "historical" && selection.record.id === record.id ? "is-active" : ""} onClick={() => chooseHistorical(record)}><strong role="cell">{record.name}</strong><span role="cell">{record.location}</span><span role="cell">{record.nfssRelated ? "NFSS-related · remediated" : "Other anomaly"}</span></button>)}</div></details>}
